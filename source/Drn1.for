...
C--SEAWAT: MODIFIED TO READ VARIABLE IDRNELEV
      READ(IN,2) MXDRN,IDRNCB,IDRNELEV
    2 FORMAT(3I10)
      WRITE(IOUT,3) MXDRN
...
C--SEAWAT: CHANGED FROM 5*MXDRN TO 6*MXDRN
      ISP=6*MXDRN
      ISUM=ISUM+ISP
C
...
C6------RETURN
      RETURN
      END
      SUBROUTINE DRN1RP(DRAI,NDRAIN,MXDRN,IN,IOUT,IDRNELEV,ELEV,NCOL,
     &           NROW,NLAY)
C
C
...
C     ------------------------------------------------------------------
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C--SEAWAT: CHANGED TO '6' AND ADDING ELEV
      DIMENSION DRAI(6,MXDRN),ELEV(NCOL,NROW,NLAY)
C     ------------------------------------------------------------------
C
...
C--SEAWAT: CHANGE READ DEPENDING ON IDRNELEV AND READ DRAIN ELEVATIONS
	IF (IDRNELEV.GT.0) THEN
		WRITE(IOUT,303)
  303		FORMAT(1H0,15X,'LAYER',5X,'ROW',5X
     1	,'COL   DRN STAGE   CONDUCTANCE       DRNELEV   DRAIN NO.'
     2    /1X,15X,74('-'))
		DO 350 II=1,NDRAIN
			READ (IN,304) K,I,J,DRAI(4,II),DRAI(5,II),DRAI(6,II)
  304			FORMAT(3I10,3F10.0)
			WRITE (IOUT,305) K,I,J,DRAI(4,II),DRAI(5,II),DRAI(6,II),II
  305			FORMAT(1X,15X,I4,I9,I8,G13.4,G14.4,G14.4,I8)
			DRAI(1,II)=K
			DRAI(2,II)=I
			DRAI(3,II)=J
  350		CONTINUE
	ELSE
C--SEAWAT: READ DRAINS THE OLD WAY AND SET DRNELEV TO CELL CENTER
		WRITE(IOUT,3)
    ...
    5			FORMAT(1X,15X,I4,I9,I8,G13.4,G14.4,I8)
			...
			DRAI(2,II)=I
			DRAI(3,II)=J
C--SEAWAT: SET DRAIN ELEVATION BY DEFAULT TO CELL CENTER ELEVATION
			DRAI(6,II)=ELEV(J,I,K)
  250		CONTINUE
	ENDIF
C
C8------RETURN
...
C--SEAWAT: CHANGED TO '6'
      DIMENSION DRAI(6,MXDRN),HNEW(NCOL,NROW,NLAY),
...
C5-------IF THE CELL IS INTERNAL GET THE DRAIN DATA.
      EL=DRAI(4,L)
C5S1--SEAWAT: USE SALTWATER HEAD FOR TURNING DRAIN ON
      HHNEW=SALTHEAD(HNEW(IC,IR,IL),PS(IC,IR,IL),ELEV(IC,IR,IL))
C
...
C7S1--SEAWAT: CHANGE TO MASS FLUX
      C=DRAI(5,L)
      HCOF(IC,IR,IL)=HCOF(IC,IR,IL)-C*RHOIJK
      RHS(IC,IR,IL)=RHS(IC,IR,IL)-RHOIJK*C*
     +              (EL-(RHOIJK-PF)/PF*(ZIJK-ZDRN))
  100 CONTINUE
C
C8------RETURN
      RETURN
     ...
     1      NCOL,NROW,NLAY,IBOUND,ntr,KSTP,KPER,IDRNCB,ICBCFL,BUFF,IOUT,
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
CS00-SEAWAT: DELT=DTRANS, NTR=TRANSPORT STEP NUMBER
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C
C--SEAWAT: CHANGED DRAI TO '6'
      DIMENSION VBNM(4,MSUM),VBVL(4,MSUM),DRAI(6,MXDRN),
...
C1------ACCUMULATORS (RATIN AND RATOUT).
      ...
      IBD=0
C1S1--SEAWAT: SET IBD
      IF (ICBCFL.NE.0.AND.IDRNCB.GT.0.AND.STEPEND) IBD=1
C
...
C7------GET DRAIN PARAMETERS FROM DRAIN LIST.
      EL=DRAI(4,L)
      C=DRAI(5,L)
C7S1--SEAWAT: USE SALTWATER HEAD
      HHNEW=SALTHEAD(HNEW(IC,IR,IL),PS(IC,IR,IL),ELEV(IC,IR,IL))
C
...
C9S1--SEAWAT: USING HNEW(IC,IR,IL) IN THE NEXT EQUATION BECAUSE HHNEW IS SALTHEAD
      Q=RHOIJK*C*(EL-HNEW(IC,IR,IL)-(RHOIJK-PF)/PF*(ZIJK-ZDRN))
      RATOUT=RATOUT-Q
C
...
C11S1--SEAWAT: STORING BUFF AS VOLUMETRIC FLUX
      IF(IBD.EQ.1) BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+Q/PS(IC,IR,IL)
  100 CONTINUE
C
