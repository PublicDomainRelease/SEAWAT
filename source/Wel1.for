...
C4S1--SEAWAT: INCREASED ARRAY SIZE FROM 4 TO 5 TO STORE DENSITY
      ISP=5*MXWELL
      ISUM=ISUM+ISP
C
...
C7------RETURN
      RETURN
      END
      SUBROUTINE WEL1RP(WELL,NWELLS,MXWELL,IN,IOUT,PS,MXSS,NSS,SS,
     +	NCOL,NROW,NLAY)
C
C
...
C0S1--SEAWAT: DIMENSION OF WELL CHANGED FROM 4 TO 5
      DIMENSION WELL(5,MXWELL)

C0S2--SEAWAT: DIMENSION SS AND PS
	DIMENSION SS(6,MXSS),PS(NCOL,NROW,NLAY)
C     ------------------------------------------------------------------
C
...
C5------READ AND PRINT LAYER,ROW,COLUMN AND RECHARGE RATE.
      WRITE(IOUT,3)
    3 FORMAT(1H ,47X,'LAYER    ROW    COL    STRESS RATE    DENSITY  ',
     1'   WELL NO.'/,48X,58('-'))
      DO 250 II=1,NWELLS
      READ (IN,4) K,I,J,Q
    4 FORMAT(3I10,F10.0)
...
C5S1--SEAWAT: STORE DENSITY OF WELL FLUID IN WELL(5,II)
	WELL(5,II)=PS(J,I,K)
	IF (Q.GT.0.) WELL(5,II)=SSMDENSE(I,J,K,2,MXSS,NSS,SS)

C5S2--SEAWAT: MOVED THIS WRITE DOWN UNTIL AFTER DENSITY WAS SET
      WRITE (IOUT,5) K,I,J,Q,WELL(5,II),II
    5 FORMAT(48X,I3,I8,I7,G16.5,G16.4,I8)
...
C6------RETURN
  260 RETURN
     ...
     1        NCOL,NROW,NLAY,PS)
C
C-----VERSION 1233 30NOV2001 WEL1FM
C
C     ******************************************************************
...
C     ------------------------------------------------------------------
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      DIMENSION RHS(NCOL,NROW,NLAY),WELL(5,MXWELL),
     1          IBOUND(NCOL,NROW,NLAY),PS(NCOL,NROW,NLAY)
C     ------------------------------------------------------------------
...
C2------PROCESS EACH WELL IN THE WELL LIST.
      ...
      IL=WELL(1,L)
      Q=WELL(4,L)
C2S1--SEAWAT: ASSIGN THE DENSITY OF THE WELL FLUID TO DENSE
	DENSE=WELL(5,L)
	IF(Q.LT.0) DENSE=PS(IC,IR,IL)
C
...
C2BS1--SEAWAT: MULTIPLY THE FLOW RATE BY DENSE
...
C3------RETURN
     ...
     1             NCOL,NROW,NLAY,ntr,KSTP,KPER,IWELCB,ICBCFL,BUFF,IOUT,
     2             PRTOUT,STEPEND,PS)
C
C-----VERSION 1509 30NOV2001 WEL1BD
C     ******************************************************************
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
CS00--SEAWAT: DELT=DTRANS NTR=TRANSPORT STEP NUMBERS
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)

      CHARACTER*4 VBNM,TEXT
      DIMENSION VBNM(4,MSUM),VBVL(4,MSUM),WELL(5,MXWELL),
     1          IBOUND(NCOL,NROW,NLAY),BUFF(NCOL,NROW,NLAY),
	2          PS(NCOL,NROW,NLAY)
      DIMENSION TEXT(4)
C
C0S1--SEAWAT: LOGICAL PRTOUT
...
C1------CLEAR RATIN AND RATOUT ACCUMULATORS.
      ...
      IBD=0
C1S1--SEAWAT: SET IBD
      IF(ICBCFL.NE.0.AND.IWELCB.GT.0.AND.STEPEND) IBD=1
C
...
C5------PROCESS WELLS ONE AT A TIME.
   60 DO 100 L=1,NWELLS
      ...
      IC=WELL(3,L)
      IL=WELL(1,L)
C5S1--SEAWAT: SET DENSITY OF WELL FLUID TO WELL(5,L)
	DENSE=WELL(5,L)
	IF(Q.LT.0) DENSE=PS(IC,IR,IL)
      Q=WELL(4,L)*DENSE
C
...
C5CS1--SEAWAT: CHANGE FROM MASS FLUX TO VOLUME FLUX FOR BUFF
      IF(IBD.EQ.1) BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+Q/DENSE
      IF(Q) 90,100,80
C
