CHD1.F77
      SUBROUTINE CHD1AL(ISUM,LENX,LCCHDS,NCHDS,MXCHD,IN,IOUT,ICHDSALT)
C
...
C--SEAWAT: MODIFIED TO READ ICHDSALT FLAG [0 MEANS HEADS ARE FRESHWATER]
C--SEAWAT: A NONZERO VALUE FOR ICHDSALT MEANS HEADS ARE ACTUAL HEADS
      READ(IN,2) MXCHD,ICHDSALT
    2 FORMAT(2I10)
      WRITE(IOUT,3) MXCHD,ICHDSALT
    3 FORMAT(1H ,'A TOTAL OF',I5,' CONSTANT-HEAD CELLS MAY BE',
     2 ' SPECIFIED EACH STRESS PERIOD.',
     3 /1X,I5,'ICHDSALT FLAG [0: HEADS ARE FRESHWATER')
C
...
CLANGEVIN COMMENTED NEXT 2 LINES, PERLEN PASSED TO MAIN PROGRAM
CLANGEVIN FROM BTN3ST
C      PERLEN=DELT*FLOAT(NSTP)
C      IF(TSMULT.NE.1.) PERLEN=DELT*(1.-TSMULT**NSTP)/(1.-TSMULT)
C8------RETURN
  260 RETURN
      END
      SUBROUTINE CHD1FM(NCHDS,MXCHD,CHDS,IBOUND,HNEW,
     1         HOLD,PERLEN,PERTIM,DELT,NCOL,NROW,NLAY,ICHDSALT,
     2         PS,ELEV)
C
...
C
	...
	3          PS(NCOL,NROW,NLAY),ELEV(NCOL,NROW,NLAY)
...
C
C2------COMPUTE PROPORTION OF STRESS PERIOD TO END OF THIS TIME STEP
      FRAC=PERTIM/PERLEN
...
C--SEAWAT: IF HEADS IN THE CHDPACKAGE WERE ENTERED AS ACTUAL HEADS, THEY
C--SEAWAT: MUST BE CORRECTED TO BE EQUIVALENT FRESHWATER HEADS
	IF (ICHDSALT.GT.0) HB=PS(IC,IR,IL)/PF*HB-
     1                    (PS(IC,IR,IL)-PF)/PF*ELEV(IC,IR,IL)
C
