     ...
     &                      NCOL,NROW,NLAY,NRIVVL,ELEV,PS,MTDNCONC,
...
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS
      DIMENSION ELEV(NCOL,NROW,NLAY),PS(NCOL,NROW,NLAY),SS(NSSVL,MXSS),
     +          SSMC(NCOMP,MXSS)
	INCLUDE 'vdf.inc'
...
C9------RETURN
     ...
     3      ELEV,PS,MTDNCONC,MXSS,NSS,SS,NCOMP,SSMC,NSSVL)
C-----VERSION 05JUNE2000 GWF1RIV6BDST
C     ******************************************************************
...
C5C-----GET RIVER PARAMETERS FROM RIVER LIST.
		...
		RIVDENS=PS(IC,IR,IL)
		IF(LOCRIVDEN.GT.0) RIVDENS=RIVR(LOCRIVDEN,L)
		IF(MTDNCONC.GT.0) RIVDENS=SSMDENSE(IC,IR,IL,4,MXSS,NSS,SS,
     &                           NCOMP,SSMC,MTDNCONC)   
		HRIV=FEHEAD(HRIV,RIVDENS,RBOT+RBDTHK)
		HHNEW=HNEW(IC,IR,IL)
