...
C--SEAWAT:-----ASSIGN VERSION NUMBER AND DATE
      ...
      PARAMETER (VERSION  ='4.00.00 03/28/2008')
	PARAMETER (VERSIONMF='1.18.00 08/23/2007')
...
C--SEAWAT: IF IUPDTFLWFLD=1)
     ...
     &                           Y(LCCOLD),NCOMP,NSWTCPL,TIME2,HT2,
     &                           IUNIT(57),IUNIT(58))
...
c      call clang(icomp,ncol,nrow,nlay,y(lccnew+(icomp-1)*nodes))
                IF(iUnitTRNOP(5).GT.0) 
...
C--SEAWAT: RESET DELT AND COLDFLW IF MODFLOW WAS SOLVED THIS TRANSPORT TIMESTEP
           IF(IUNIT(57).GT.0.AND.IUNIT(58).GT.0.AND.IUPDTFLWFLD.EQ.1) 
...
C
      ...
      subroutine clang(icomp,ncol,nrow,nlay,cnew)
      real temp(530,1,275)
      dimension cnew(ncol,nrow,nlay)
      if(icomp.eq.1) iunit=91
      if(icomp.eq.2) iunit=92
      write(iunit,*) 'icomp=',icomp
      if(icomp.eq.1)
     +write(iunit,100) (((cnew(j,i,k),j=1,ncol),i=1,nrow),k=1,nlay)
      if(icomp.eq.2) then
        temp=cnew
        do k=1,nlay
        do i=1,nrow
        do j=1,ncol
            if(temp(j,i,k).gt.-0.5) temp(j,i,k)=1.-temp(j,i,k)
        enddo
        enddo
        enddo
       write(iunit,100) (((temp(j,i,k),j=1,ncol),i=1,nrow),k=1,nlay)
      endif
  100 format(10g15.5)
      end
