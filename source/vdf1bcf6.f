     ...
     3                WETFCT,IWETIT,IHDWET,HDRY,BUFF,PS,ELEV,HSALT)
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
C--SEAWAT: ADD HTMP,HN,HNFE, HSALT TO DOUBLE PRECISION 
      DOUBLE PRECISION HNEW,HTMP,HN,HNFE,HSALT
C
...
C     1                        - CV(J,I,K)*(BOTM(J,I,LBOTM(K+1)-1)-HTMP)
      IF(SALTHEAD(HTMP,PS(J,I,K+1),ELEV(J,I,K+1)).LT.
     &   BOTM(J,I,LBOTM(K+1)-1)) 
     &   RHS(J,I,K)=RHS(J,I,K) + PS(J,I,K)*
     &                 CV(J,I,K)*(HTMP-ELEV(J,I,K+1)+PS(J,I,K)/DENSEREF
     &                  *(ELEV(J,I,K+1)-BOTM(J,I,LBOTM(K+1)-1)))
...
C10-----RETURN.
     ...
     2   ICBCFL,BUFF,IOUT,PERTIM,TOTIM,PS,ELEV,HSALT)
...
C--SEAWAT: ADD HTMP,HSALT
      DOUBLE PRECISION HNEW,STOIN,STOUT,SSTRG,HSING,HTMP,HSALT
C
      DIMENSION HNEW(NCOL,NROW,NLAY), IBOUND(NCOL,NROW,NLAY),
...
C11----RETURN.
     ...
     2         PERTIM,TOTIM,BOTM,NBOTM,ICHFLG,PS,ELEV,MFNADVFD,DELR,
...
C--SEAWAT: ADD H1,HS1.H2,HS2,HSALT
      DOUBLE PRECISION HNEW(NCOL,NROW,NLAY),HD,CHIN,CHOUT,XX1,XX2,XX3,
     &                 XX4,XX5,XX6,H1,HS1,H2,HS2,HSALT(NCOL,NROW,NLAY),
     &                 FLOWDIR
C
...
C18-----RETURN.
     ...
     3      ELEV,PS,DELR,DELC,IWTABLE,HSALT)
C
...
C
     ...
     3     BUFF(NCOL,NROW,NLAY),ELEV(NCOL,NROW,NLAY),
     4     PS(NCOL,NROW,NLAY),DELR(NCOL),DELC(NROW)
C
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
      COMMON /BCFCOM/LAYCON(999)
	INCLUDE 'vdf.inc'
