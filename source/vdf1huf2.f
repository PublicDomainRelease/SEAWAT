...
C--SEAWAT: PASS IN HSALT
     ...
     & ILVDA,VDHD,VDHT,IWETIT,IHDWET,WETFCT,GS,A9,HSALT)
C
C     ******************************************************************
...
C--SEAWAT: PASS HSALT, PS, ELEV
     ...
     &    IWETIT,IHDWET,WETFCT,GS,A9,HSALT,PS,ELEV)
C     ******************************************************************
...
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS AND INCLUDE vdf.inc
	DIMENSION HSALT(NCOL,NROW,NLAY),PS(NCOL,NROW,NLAY),
     &    ELEV(NCOL,NROW,NLAY)
	INCLUDE 'vdf.inc'
C
...
C--SEAWAT: CALL VDF1HUF2SP AND PASS IN HSALT
        CALL VDF1HUF2SP(
     &   IBOUND,HNEW,CR,CC,CV,DELR,DELC,BOTM,HK,VKA,SC1,
...
C--SEAWAT: USE VARIABLE-DENSITY FORM OF CORRECTION TERM
     ...
     &      (ELEV(J,I,K)-HTMP+PS(J,I,K-1)/DENSEREF*
     &      (TOP-ELEV(J,I,K)))
  220    CONTINUE
      END IF
C
...
C--SEAWAT: USE VARIABLE-DENSITY FORM OF CORRECTION TERM
     ...
     &        (HTMP-ELEV(J,I,K+1)+PS(J,I,K)/DENSEREF*
     &        (ELEV(J,I,K+1)-TOP))
  280    CONTINUE
      END IF
C
...
c======================================================================
     ...
     &         PERTIM,TOTIM,NBOTM,ICHFLG,ILVDA,VDHT,PS,ELEV,MFNADVFD,
...
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS AND INCLUDE vdf.inc
	DIMENSION PS(NCOL,NROW,NLAY),ELEV(NCOL,NROW,NLAY),
     &     HSALT(NCOL,NROW,NLAY),DELR(NCOL),DELC(NROW)
	INCLUDE 'vdf.inc'
C
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
      COMMON /HUFCOM/LTHUF(999),HGUHANI(999),HGUVANI(999),LAYWT(999)
...
C--SEAWAT: PASS IN ADDITIONAL VARIABLES
     ...
     &      PS,ELEV,DELR,DELC,IWTABLE,HSALT)
C
C     ******************************************************************
...
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS AND INCLUDE vdf.inc
	DIMENSION PS(NCOL,NROW,NLAY),ELEV(NCOL,NROW,NLAY),
     &     DELR(NCOL),DELC(NROW),HSALT(NCOL,NROW,NLAY)
	INCLUDE 'vdf.inc'
C
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
      COMMON /HUFCOM/LTHUF(999),HGUHANI(999),HGUVANI(999),LAYWT(999)
...
C--SEAWAT: PASS PS AND ELEV
     ...
     &                   ICHFLG,HSALT,PS,ELEV)
      CALL SVDF1HUF2FLOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
     &                   IZON,NZONAR,IOUT,GS,IOHUFFLWS,ICBCFL,KSTP,KPER,
     &                   DELT,PERTIM,TOTIM,ICHFLG,HSALT,PS,ELEV)
      ENDIF
C
C6------RETURN.
      RETURN
...
C     ------------------------------------------------------------------
     ...
     &    HUFTHK(NCOL,NROW,NHUF,2),RMLT(NCOL,NROW,NMLTAR),
     &    IZON(NCOL,NROW,NZONAR),GS(NCOL,NROW)
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS, INCLUDE vdf.inc, AND COMMON DISCOM
	DIMENSION HSALT(NCOL,NROW,NLAY),PS(NCOL,NROW,NLAY),
     &    ELEV(NCOL,NROW,NLAY)
	INCLUDE 'vdf.inc'
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
...
C
     ...
     &     HUFTHK(NCOL,NROW,NHUF,2),RMLT(NCOL,NROW,NMLTAR),
     &     IZON(NCOL,NROW,NZONAR),GS(NCOL,NROW)
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS, INCLUDE vdf.inc
	DIMENSION HSALT(NCOL,NROW,NLAY),PS(NCOL,NROW,NLAY),
     &    ELEV(NCOL,NROW,NLAY)
	INCLUDE 'vdf.inc'
C
