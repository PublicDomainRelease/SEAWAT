...
C--SEAWAT:  LANGEVIN 10/28/2004 FIXED A BUG UCOLNO THAT CAUSED SEAWAT TO
C--SEAWAT:    BOMB IF THE NUMBER OF COLUMNS EXCEEDED 1000
C--SEAWAT:  LANGEVIN 10/28/2004 ADDED TWO NEW PARAMETERS, VERSIONMF, AND
C--SEAWAT:    VERSIONMT, WHICH CONTAIN THE MODFLOW AND MT3DMS VERSIONS
C--SEAWAT:  LANGEVIN 03/07/2005 UPDATED WITH MF2K VERSION 1.15 AND 
C--SEAWAT:    MT3DMS VERSION 5.00
C--SEAWAT:  LANGEVIN 03/07/2005 MODIFIED IMT1FMI5RP1 AND RP2 SUCH THAT
C--SEAWAT:    TIMESTEP LENGTHS ARE NOT RESTRICTED BY STABILITY CRITERIA
C--SEAWAT:    CALCULATED AT CONSTANT CONCENTRATION CELLS
C--SEAWAT:  LANGEVIN 03/22/2005 ADDED THE LMG1.F AND AMG1R6.F FILES TO
C--SEAWAT:    PRODUCE AN INTERNAL USGS VERSION OF THE CODE
C--SEAWAT:  LANGEVIN 04/07/2005 MODIFIED MNW TO WORK WITH VDF.  CURRENTLY,
C--SEAWAT:    PUMPED WATER IS ASSUMED TO HAVE A CONCENTRATION OF ZERO
C--SEAWAT:  LANGEVIN 04/12/2005 MNW LINK ROUTINE WAS NOT WORKING PROPERLY
C--SEAWAT:    SO MODIFIED MNW SUBROUTINES AND LINK SUBROUTINES SO THAT
C--SEAWAT:    GROUP INFORMATION WAS STORED AND PASSED PROPERLY.
C--SEAWAT:  LANGEVIN 04/28/2005 FOUND BUG THAT ALLOWED SEAWAT TO CONTINUE
C--SEAWAT:    EVEN IF FLOW SOLUTION HAD NOT CONVERGED
C--SEAWAT:  LANGEVIN 04/29/2005 PREPARED CODE FOR PUBLIC DISTRIBUTION AND
C--SEAWAT:    AND RELEASED ON WEB
C--SEAWAT:  LANGEVIN 07/25/2005 FIXED BUG CAUSED BY THE UPGRADE TO 
C--SEAWAT:    MT3DMS 5.00.  SS ARRAY NEEDED TO CHANGE FROM SIZE 6 TO 7.
...
C--SEAWAT:  LANGEVIN 04/21/2006 CHANGED TO VERSION 3.13 AND UPGRADED TO
C--SEAWAT:     MODFLOW VERSION 1.16.00; ALSO PROGRAMMED ETS AND DRT TO
C--SEAWAT:     WORK WITH VARIABLE-DENSITY FLOW (VDF) PROCESS
...
C--SEAWAT:-----ASSIGN VERSION NUMBER AND DATE
      ...
      PARAMETER (VERSION  ='3.13.00-PUBLIC 04/21/2006')
	PARAMETER (VERSIONMF='1.16.00 03/28/2006')
	PARAMETER (VERSIONMT='5.10    10/25/2005')
C
...
C  STATEMENTS MUST BE COMMENTED OUT
  ...
  700   FORMAT(1X,'ALLOCATION OF ARRAYS GX, GZ, AND IG FAILED,',
     &  ' RETURNED ERROR MESSAGE NUMBER: ',I6)
        CALL USTOP(' ')
      ENDIF
C--SEAWAT: FORCE IG TO ZERO, OTHERWISE PROGRAM MAY BOMB WITH SOME COMPILERS
	DO I=1,LENIG
		IG(I)=0
	ENDDO
C
...
C--SEAWAT: READ AND PREPARE MT3DMS INFORMATION (CONSTANT FOR ENTIRE SIMULATION)
     ...
     & Y(LCHTOP),Y(LCDZ),Y(LCPR),IY(LCIB),Y(LCCOLD),Y(LCCNEW),
     ...
     & MCOMP,MIXELM,MXPART,NADVFD,NCOUNT)
      IF(iUnitTRNOP(2).GT.0) 
     & CALL IMT1DSP5RP(iUnitTRNOP(2),IOUT,NCOL,NROW,NLAY,
     & Y(LCAL),Y(LCTRPT),Y(LCTRPV),Y(LCDM))
      IF(iUnitTRNOP(4).GT.0) 
     & CALL IMT1RCT5RP(iUnitTRNOP(4),IOUT,NCOL,NROW,NLAY,
     & NCOMP,IY(LCIB),Y(LCCOLD),Y(LCPR),ISOTHM,IREACT,IRCTOP,IGETSC,
...
C--SEAWAT: STRESS TIMING CONTROLLED BY MT3DMS
         IF(IUNIT(58).GT.0) 
     &    CALL IMT1BTN5ST(IUNIT(58),IOUT,NSTP,MXSTP,TSLNGH,DT0,
...
C--SEAWAT: PASS IN PERLEN INSTEAD OF DELT
     ...
     &	   PERLEN(KKPER),KSTP,NSTP,MXPRS,TIMPRS,DT0,MXSTRN,MIXELM,
     &	   DTRACK,DTRACK2,PERCEL,DTDISP,DTSSM,DTRCT,RFMIN,NPRS,NPS,
     &	   DTRANS,PRTOUT,NCOL,NROW,NLAY,NCOMP,IY(LCIB),Y(LCCNEW),
     &	   Y(LCCOLD),CINACT,UPDLHS,IMPSOL,TTSMULT,TTSMAX,KPER,
     &	   Y(LTDELR),Y(LTDELC),Y(LCDH),Y(LCPR),Y(LCSR),Y(LCRHOB),
...
C--SEAWAT:IF(IUNIT(57).GT.0) PASS RHOCR, RHOCC, AND RHOCV
     ...
     &                        IOUTAMG,ICG,IADAMP,DUP,DLOW)
              IF (IUNIT(42).GT.0)
     &            CALL GMG1AP(GZ(LCHNEW),GX(LCRHS),VDF(LRHOCR),
     &                        VDF(LRHOCC),VDF(LRHOCV),GX(LCHCOF),HNOFLO,
...
C--SEAWAT: COMPUTING IEVT ARRAY
     ...
     &        RX(LCPXDP),RX(LCPETM),NSEGAR,VDF(LPS),VDF(LELEV),INUHF)
     ...
     &          RX(LCWEL2),IG(LCIBOU),NCOL,NROW,NLAY,NODES,KKSTP,KKPER,
     &          INUHF,INUHF)
            IF(IUNIT(40).GT.0) CALL LMT6DRT1VD(NDRTCL,MXDRT,RX(LCDRTF),
     &       GZ(LCHNEW),NCOL,NROW,NLAY,IG(LCIBOU),KKSTP,KKPER,
     &       INUHF,NDRTVL,IDRTFL,NRFLOW,VDF(LPS),VDF(LELEV),INUHF)
      ELSE
...
C--SEAWAT: COMPUTE IEVT ARRAY
     ...
     &          NETSEG,RX(LCPXDP),RX(LCPETM),NSEGAR,INUHF)
...
C--SEAWAT: PASS IN A 1 FOR ILMTHEAD
     ...
     &          GZ(LCHNEW),NCOL,NROW,NLAY,IG(LCIBOU),KKSTP,KKPER,
     &          INUHF,NDRTVL,IDRTFL,NRFLOW,INUHF)
      ENDIF
      REWIND(INUHF)

C--SEAWAT: READ FLOW INFORMATION FROM TEMPORARY FILE
       CALL IMT1FMI5AL(INUHF,IOUT,MXTRNOP,iUnitTRNOP,NPERFL,ISS,IVER)
          CALL IMT1FMI5RP1(INUHF,IOUT,KPER,KSTP,NCOL,NROW,NLAY,
     &     NCOMP,FPRT,IY(LCLAYC),IY(LCIB),HORIGN,Y(LCDH),Y(LCPR),
...
C--SEAWAT: CALCULATE COEFFICIENTS THAT ARE CONSTANT WITHIN EACH TRANSPORT STEP
          IF(iUnitTRNOP(2).GT.0) 
     &     CALL IMT1DSP5CF(IOUT,KSTP,KPER,NCOL,NROW,NLAY,
...
C--VARIABLE DENSITY DRT PACKAGE
     ...
     &                          IG(LCIBOU),KKSTP,KKPER,IDRTCB,ICBCFL,
     &                          GX(LCBUFF),IOUT,PERTIM,TOTIM,NDRTVL,
     &                          IDRTAL,IDRTFL,NRFLOW,IAUXSV,VDF(LPS),
     &                          VDF(LELEV))
            IF(IUNIT(51).GT.0)
...
C--SEAWAT: CALCULATE GLOBAL MASS BUDGETS AND CHECK MASS BALANCE
     ...
     &         ISS,IY(LCIB),Y(LTDELR),Y(LTDELC),Y(LCDH),Y(LCPR),
